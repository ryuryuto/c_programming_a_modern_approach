...
void changePrice(struct part inventory[], int num_parts);
...
int main(void) {
    ...
    for (;;) {
        printf("i:Insert, s:Search, u:Update, p:Print, c:Change Price, q:Quit\n");
        ...
        switch (code) {
            ...
            case 'p':
                ...
                break;
            case 'c':
                changePrice(inventory, num_parts);
                break;
            case 'q':
                return 0;
            default:
 ...
 **********************************************************/
 /**
  * struct虽然是值传递，但这里因为是数组，所以传递的是地址
  * @param number
  * @param inventory
  * @param num_parts
  * @return
